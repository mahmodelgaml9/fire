-- =====================================================
-- جداول لوحة التحكم - Sphinx Fire CMS (محدث)
-- =====================================================
...
-- =====================================================
-- إضافة حقول جديدة للجداول الموجودة (بدون تكرار)
-- =====================================================
...
SET @sql = (SELECT IF(
    ...
    'ALTER TABLE `pages` ADD COLUMN `updated_by` INT DEFAULT NULL AFTER `created_by`',
    'SELECT "Column updated_by already exists in pages table"'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = (SELECT IF(
    ...
    'ALTER TABLE `pages` ADD COLUMN `is_featured` BOOLEAN DEFAULT FALSE AFTER `updated_by`',
...
-- إضافة الفهارس للجداول الموجودة
ALTER TABLE `pages` 
